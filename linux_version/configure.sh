...
if [ ! -f "nginx_certs/domain.key" ] && [ ! -f "nginx_certs/domain.crt" ]; then
    ...
    -addext "subjectAltName = IP:192.168.56.102" \
...
# Get docker to trust self signed certs
sudo mkdir -p "/etc/docker/certs.d/192.168.56.102"
sudo cp ./nginx_certs/server.crt /etc/docker/certs.d/192.168.56.102/ca.crt